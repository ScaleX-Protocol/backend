...
function dropAndRecreateDatabase(envFile: string) {
	...
	try {
		...
		execSync(
			`docker exec postgres-database psql -U ${user} -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = '${dbName}' AND pid <> pg_backend_pid();"`,
			{ stdio: "inherit" }
		);

		// Drop the database
		console.log("Dropping database...");
		execSync(`docker exec postgres-database psql -U ${user} -c "DROP DATABASE IF EXISTS ${dbName};"`, {
			stdio: "inherit",
		});
	...
	try {
		...
		execSync(`docker exec postgres-database psql -U ${user} -c "CREATE DATABASE ${dbName};"`, {
			stdio: "inherit",
		});
