...
export async function handleOrderMatched({ event, context }: any) {
  ...
  const buyOrder = await findActiveOrder(db, chainId, poolAddress, BigInt(args.buyOrderId));
  if (buyOrder) {
    ...
    const buyOrderUpdated = await updateOrderQuantity(db, chainId, buyOrder.id, quantity, price);
  ...
  const sellOrder = await findActiveOrder(db, chainId, poolAddress, BigInt(args.sellOrderId));
  if (sellOrder) {
    ...
    const sellOrderUpdated = await updateOrderQuantity(db, chainId, sellOrder.id, quantity, price);
...
export async function handleOrderCancelled({ event, context }: any) {
  ...
  const order = await findActiveOrder(db, chainId, poolAddress, BigInt(event.args.orderId!));
  ...
  try {
    const orderUpdateSuccess = await updateOrder(db, chainId, order.id, event, timestamp);
...
export async function handleUpdateOrder({ event, context }: any) {
  ...
  // Find order by natural keys (since orderId may have been reused)
  const order = await findActiveOrder(db, chainId, poolAddress, orderId);
