...
export async function handleOrderPlaced({ event, context }: any) {
  ...
  try {
    ...
    try {
      ...
      log(LogLevel.ERROR, 'Order insertion failed', LogLabel.DATABASE, ServiceName.CORE_CHAIN, {
        error: error instanceof Error ? error.message : String(error),
        orderId: args.orderId,
        poolAddress,
        chainId
...
export async function handleOrderCancelled({ event, context }: any) {
  ...
  try {
    ...
    log(LogLevel.ERROR, 'OrderCancelled error', LogLabel.EVENT_HANDLER, ServiceName.CORE_CHAIN, {
      error: e instanceof Error ? e.message : String(e),
      hashedOrderId,
      orderId: event.args.orderId,
      poolAddress
    }, 'orderBookHandler.ts', 'handleOrderCancelled');
    return;
  }
}
