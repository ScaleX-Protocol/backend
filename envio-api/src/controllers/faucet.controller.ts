...
import { faucetDb } from '../config/database';
import { faucetRequests } from '../schema';
...
export class FaucetController {
  ...
  private async recordFaucetRequest(request: any): Promise<number | null> {
    try {
      const result = await faucetDb.insert(faucetRequests).values(request).returning({ id: faucetRequests.id });
