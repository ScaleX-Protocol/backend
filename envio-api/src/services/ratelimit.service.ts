...
import { faucetRateLimits } from '../schema';
...
export class RateLimitService {
  ...
  public async setCooldown(identifier: string, identifierType: 'address' | 'ip', cooldownMs: number): Promise<void> {
    ...
    await faucetDb.insert(faucetRateLimits)
      .values({
  ...
  private async checkRateLimit(
    ...
    const record = await faucetDb.query.faucetRateLimits.findFirst({
      where: and(
  ...
  private async consumeRateLimit(
    ...
    // Increment counter
    await faucetDb.update(faucetRateLimits)
      .set({
